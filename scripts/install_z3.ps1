...
mkdir build
cd build
cmake -G "Visual Studio 17 2022" -DZ3_BUILD_LIBZ3_SHARED=OFF -DCMAKE_INSTALL_PREFIX="$githubWorkspace\build\deps" -DCMAKE_BUILD_TYPE=Release ..
cmake --build . --config Release -j 12
cmake --install .

# Export to environment variable
"Z3_DIR=$githubWorkspace\build\deps\lib\cmake\z3" | Out-File -Append -FilePath $Env:GITHUB_ENV
cd ..\..
